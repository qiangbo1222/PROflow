...
from dataset.process_mols import lig_feature_dims, rec_residue_feature_dims
    ...
    def forward(self, x):
        ...
        if self.lm_embedding_type is not None:
            if len(x.shape) == 2:
                ...
                x_embedding = self.lm_embedding_layer(torch.cat([x_embedding, x[:, :, -self.lm_embedding_dim:]], axis=2))
        return x_embedding
...
class TensorProductConvLayer(torch.nn.Module):
    ...
    def forward(self, node_attr, edge_index, edge_attr, edge_sh, out_nodes=None, reduce='mean'):
        ...
        edge_sh = edge_sh.to(torch.float32)
        ...
        if self.residual:
            padded = F.pad(node_attr[edge_src], (0, out.shape[-1] - node_attr.shape[-1]))
            out = out + padded
...
class TensorProductMLPLayer(torch.nn.Module):#This Net module cause the in-place error
    def __init__(self, in_irreps, out_irreps, dropout=0.0,
                 hidden_features=None):
        super(TensorProductMLPLayer, self).__init__()
        # a polinomial layer + a relu + a dropout
        self.in_irreps = in_irreps
        self.out_irreps = out_irreps
        
        
        if hidden_features is None:
            hidden_features = in_irreps
        
        self.linear = o3.Linear(in_irreps, out_irreps)
        out_dim = int(o3.Irreps(out_irreps).dim)

        self.gate = o3.Linear(in_irreps, f'{out_dim}x0e')
    ...
    def forward(self, input_x):
        ...
        gate_ = self.gate(input_x)
        gate_c = self.fc(gate_)
        x = x * gate_c
        return x
...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        print("lig shape", lig_node_attr.shape, lig_edge_attr.shape)
        ...
        print("rec shape", rec_node_attr.shape, rec_edge_attr.shape)
