...
def get_transformation_mask(pyg_data, mol):
    mol = Chem.RemoveHs(mol)
    to_rotate = []
    edges = pyg_data['ligand', 'lig_bond', 'ligand'].edge_index.T.numpy()
    ...
    for i in range(0, edges.shape[0], 2):
        ...
        if not nx.is_connected(G2):
            bond = mol.GetBondBetweenAtoms(int(edges[i, 0]), int(edges[i, 1]))
            if bond is None:
                print("got error at mol:", Chem.MolToSmiles(mol))
