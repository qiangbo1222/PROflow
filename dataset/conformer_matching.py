...
def optimize_rotatable_bonds(mol, true_mol, rotable_bonds, probe_id=-1, ref_id=-1, seed=0, popsize=20, maxiter=80,
                             mutation=(0.5, 1), recombination=0.8, workers=1, return_angle=False):
    ...
    opt_mol = apply_changes(org_mol, result['x'], opt.rotable_bonds, conf_id=probe_id)

    rmsd_match = Chem.rdMolAlign.GetBestRMS(opt_mol, true_mol, probe_id, ref_id)
    results_transform = np.array(result['x']) - np.array([GetDihedral(org_mol.GetConformer(probe_id), r) for r in rotable_bonds])
    if return_angle:
        return opt_mol, results_transform, rmsd_match
    else:
        return opt_mol
...
class OptimizeConformer:
    ...
    def score_conformation(self, values):
        ...
        try:
            ...
            print("unmatch mol:", Chem.MolToSmiles(self.mol), Chem.MolToSmiles(self.true_mol))
...
def get_torsion_angles(mol):
    ...
    for e in G.edges():
        ...
        if len(l) < 2: continue
        if mol.GetBondBetweenAtoms(e[0], e[1]).GetBondType() != Chem.BondType.SINGLE: continue# add this to avoid rigid bonds
